...
- **Owner**: HUMMBL Team
- **Stewardship scope**: Canonical Base120 registry, executable corpus contract, governance specs, v1 reference artifacts, and the pre-release active Python v2 SDK.

## Lifecycle

- **Status**: Active public repository with frozen v1 registry/corpus artifacts and a pre-release active Python v2 SDK (`base120` 2.0.0).
- **Default branch**: `main`.
...
- `base120/` contains the pre-release active v2 SDK package.
...
- The Python v1 validator package is retired, while the v2 SDK is pre-release active as a source-install package. Keep README/API docs aligned with the actual `Engine`, `Ledger`, CLI, and MCP entry points.
...
- **Visibility**: public
- **Primary function**: canonical Base120 registry, executable corpus contract, and pre-release active stdlib-only Python SDK
