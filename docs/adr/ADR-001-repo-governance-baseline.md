# ADR-001 — base120 repo governance baseline
...
A live audit of all 91 `hummbl-dev` repositories found that `hummbl-dev/base120` had a strong `AGENTS.md` but was missing the core governance artifact stack: `CONSTITUTION.md`, `KRINEIA.md`, `hummbl.repo.yaml`, `CODEOWNERS`, `_receipts/`, `docs/adr/`, `docs/handoffs/`.
...
Adopt the HUMMBL Repo Standard v0.1 artifact stack for `hummbl-dev/base120`.
...
- **Positive:** 8 protected invariants are now constitutionally protected. Weakening them requires a constitutional amendment + receipt + human approval.
